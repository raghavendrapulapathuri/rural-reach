...
const HomePage: React.FC = () => {
    ...
    const renderSchemeCard = (scheme: typeof MOCK_SCHEMES[0]) => (
        <div key={scheme.id} className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 border border-gray-200">
            ...
            <div className="p-6">
                ...
                <Link to="/schemes" className="inline-block mt-4 text-primary-600 hover:text-primary-700 font-semibold">{translate('readMore')} &rarr;</Link>
            </div>
        </div>
    );

    const renderToolCard = (tool: typeof MOCK_TOOLS[0]) => (
         <div key={tool.id} className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 border border-gray-200">
            ...
            <div className="p-6">
                ...
                <p className="text-gray-600 text-sm line-clamp-3">{tool.description}</p>
                 <Link to="/tools" className="inline-block mt-4 text-primary-600 hover:text-primary-700 font-semibold">{translate('readMore')} &rarr;</Link>
            </div>
        </div>
    );

    const renderLearningCard = (material: typeof MOCK_LEARNING[0]) => (
        <div key={material.id} className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 border border-gray-200">
            <img src={material.mediaURL} alt={material.title} className="w-full h-48 object-cover"/>
            <div className="p-6">
                 <span className="text-xs font-semibold uppercase text-primary-600">{material.type}</span>
                <h3 className="text-xl font-bold text-gray-800 mt-1 mb-2">{material.title}</h3>
                <p className="text-gray-600 text-sm line-clamp-3">{material.description}</p>
                <Link to="/learning" className="inline-block mt-4 text-primary-600 hover:text-primary-700 font-semibold">{translate('readMore')} &rarr;</Link>
            </div>
        </div>
    );
  ...
  return (
    <div className="space-y-16">
      ...
      {/* Featured Schemes */}
      <section>
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-8">{translate('schemes')}</h2>
      ...
      {/* Featured Tools */}
      <section>
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-8">{translate('tools')}</h2>
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      {/* Featured Learning */}
      <section>
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-8">{translate('learning')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
